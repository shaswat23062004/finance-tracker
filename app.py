...
# # SQLAlchemy 2.0+ requires the prefix to be 'postgresql://' not 'postgres://'
...
# --- DATABASE MODELS ---
# These replace your raw SQL CREATE TABLE statements
class User(db.Model):
    __tablename__ = 'users'
...
def monthly_spending_data():
    if 'username' in session:
        ...
        for t in transactions:
            month = t.date[:7] # Extracts 'YYYY-MM'
            monthly_totals[month] += t.amount
            
        labels = [datetime.strptime(m, '%Y-%m').strftime('%b %Y') for m in monthly_totals.keys()]
...
def statistics():
    ...
    if user_id:
        ...
        # Sort categories to find top 5
        sorted_categories = dict(sorted(expense_by_category.items(), key=lambda item: item[1], reverse=True)[:5])

        return render_template('statistics.html', 
                            total_expenses=total_expenses, 
                            expense_by_category=expense_by_category,
                            top_spending_categories=sorted_categories)
    return redirect(url_for('login'))
